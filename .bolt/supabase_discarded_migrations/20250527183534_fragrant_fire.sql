/*
  # Create remaining tables for CRM system
...
CREATE TABLE IF NOT EXISTS public.client_phones (
  ...
  created_at timestamptz DEFAULT now(),
  updated_at timestamptz DEFAULT now(),
  CONSTRAINT client_phones_type_check CHECK (type = ANY (ARRAY['MAIN'::text, 'MOBILE'::text, 'WHATSAPP'::text, 'WORK'::text, 'OTHER'::text]))
);
...
CREATE TABLE IF NOT EXISTS public.deals (
  ...
  created_at timestamptz DEFAULT now(),
  updated_at timestamptz DEFAULT now(),
  CONSTRAINT deals_probability_check CHECK (probability >= 0 AND probability <= 1)
);
...
CREATE TABLE IF NOT EXISTS public.calendar_events (
  ...
  created_at timestamptz DEFAULT now(),
  updated_at timestamptz DEFAULT now(),
  CONSTRAINT calendar_events_type_check CHECK (type = ANY (ARRAY['MEETING'::text, 'TASK'::text, 'REMINDER'::text, 'DEAL'::text, 'OTHER'::text])),
  CONSTRAINT calendar_events_priority_check CHECK (priority = ANY (ARRAY['LOW'::text, 'MEDIUM'::text, 'HIGH'::text, 'URGENT'::text])),
  CONSTRAINT calendar_events_status_check CHECK (status = ANY (ARRAY['PENDING'::text, 'IN_PROGRESS'::text, 'COMPLETED'::text, 'CANCELLED'::text]))
);
